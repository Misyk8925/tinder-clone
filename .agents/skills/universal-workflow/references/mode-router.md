...
Choose one mode before creating workflow artifacts. Route by the nature of the requested delivery, not by the number of files it may touch. Then resolve `references/project-profile.md` and use the mode-sized search in `references/efficient-project-search.md`. Immediately show the routing ledger from `references/phase-ledger.md`: the selected mode, and every phase/sub-step this mode will not run as `Mode-omit`. Do not wait until a later phase to mention skipped ceremony.
...
Run phases 1-5. Phase 1 has its concept approval. Phases 2 and 3 form one design package: draft the technical contracts in phase 2, add executable behavioural contracts in phase 3, run the combined consistency check, then request one approval for both. After each run phase, do the phase-exit review in `references/phase-exit-review.md`; do not turn phase 2 into a third gate.
...
Track this as one bug item in the selected tracker. Do not require concept documents, milestones, a full feature tree, or unrelated contract/test artifacts. Ask for approval before coding only when intended behaviour, a contract change, or risk acceptance needs an owner decision. Keep B.1–B.8 and the applicable P4/P5 rows on the ledger; `Mode-omit` the phase-1 concept ceremony at routing instead of dropping those rows later. One phase-exit review after the fix loop; do not invent phase-1–3 review sessions.
...
Implement one slice, run the applicable checks, and perform one targeted review. Do not create concept documents, a feature/project tracking container, phase milestones, or empty phase folders. Reuse existing repo locations for tests and contracts instead of duplicating them under `docs/features/`. Keep C.1–C.6 and the one-slice P4/P5 rows on the ledger; list unused phase-1–3 ceremony as `Mode-omit` at routing. If a compact field does not apply, mark that row `N/A` with a reason — do not leave it off. One phase-exit review at the end of this loop; do not run the five full-feature review sessions.
