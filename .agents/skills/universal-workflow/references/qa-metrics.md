...
|---|---|---|
| Acceptance traceability | Release-scope FRs and contract error rows with at least one linked executable acceptance check / total release-scope FRs and error rows. List any manual or blocked verification separately. | Measures whether the agreed behaviour can actually be checked, rather than code volume or test count. |
| Test-run health | For each risk-selected applicable suite: level, passed, failed, skipped, blocked, duration, and evidence link. Add a repeat-failure count when a suite is suspected flaky. | Makes a green result auditable without requiring irrelevant test levels, and exposes missing runtime-dependent coverage. |
