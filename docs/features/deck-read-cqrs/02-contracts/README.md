...
| Events | [`docs/contracts/events/deck-read.asyncapi.yaml`](../../../contracts/events/deck-read.asyncapi.yaml) | [`deck-read.asyncapi.md`](../../../contracts/events/deck-read.asyncapi.md) | `ruby scripts/validate-deck-read-contracts.rb` | Adds `deck.built.v1` and the internal materialization queue; existing `swipe-saved` and `match.created` wire payloads are unchanged. |
