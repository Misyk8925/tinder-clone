...
| FR-6 | Successful fresh import; unseen then repeats on successful rebuild; empty unseen + repeats is DEGRADED; successful empty source stays BUILDING before 30s and becomes EMPTY after 30s without a failure; first/second/30-second failure thresholds; independent repeat readiness; unavailable result when no safe repeat exists; Refresh on empty returns BUILDING | `DeckSnapshotBuilderAcceptanceTest`, `DeckRepeatFillTest`, `DeckQueryServiceMaterializedAcceptanceTest` |
