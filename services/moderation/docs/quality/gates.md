...
| Performance | reproducible local load probe | NFR-1 | Partial: warmed HTTP/in-memory p95 1072 ms; PostgreSQL run blocked |
...
|---|---|---|---|---|
| `POST /internal/v1/moderations` | p95 <= 2 s | none | Partial: 1072 ms | Warm paced 50 RPS through HTTP/Basic auth with 1 s provider stub and in-memory store; JDBC run unavailable |
